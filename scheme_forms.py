...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        func = LambdaProcedure(signature.rest, expressions.rest, env)
        env.bindings[name] = func
        return name
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    # BEGIN PROBLEM 14
    def append_to_end(pair, val):
        while pair.rest != nil:
            pair = pair.rest
        pair.rest = Pair(val, nil)
    ...
    while bindings != nil:
        ...
        try:
            ...
            if names == nil:
                ...
                append_to_end(names, name)
            val = eval_all(binding.rest, env)
            if val == nil:
                raise SchemeError()
            if vals == nil:
                vals = Pair(val, nil)
            else:
                append_to_end(vals, val)
        except:
            validate_form(binding, 2)
            if scheme_symbolp(binding.rest.first):
                name = binding.first
                if names == nil:
                    names = Pair(name, nil)
                else:
                    names.rest = Pair(name, nil)
                val = eval_all(binding.rest, env)
                if vals == nil:
                    vals = Pair(val, nil)
                else:
                    vals.rest = Pair(val, nil)
            else:
                raise SchemeError()
        bindings = bindings.rest
    print("DEBUG: names " + str(names))
    print("DEBUG: vals " + str(vals))
